...
Semantic retrieval builds the same `AwardSelection` from hit IDs via `select_awards`. Fuzzy discovery lives in the [Semantic Search Contract](semantic.md). Relationship expansion from a selection lives in the [Relationship Traversal Contract](traversal.md). The model-facing session that composes these primitives lives in the [Evidence Tooling Contract](evidence.md).
